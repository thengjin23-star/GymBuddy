...
const WorkoutLog: React.FC<WorkoutLogProps> = ({ onSave, history = [], onDelete }) => {
  ...
  const getWorkoutsForDate = (date: Date) => {
    return history.filter(session => isSameDay(parseISO(session.date), date));
  };
    ...
    <div className="flex flex-col h-full pb-24 space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-display font-bold text-white tracking-tight">
          {viewMode === 'log' ? '紀錄訓練' : '訓練日曆'}
        </h2>
        <div className="flex bg-surface/50 p-1 rounded-xl border border-white/5">
          <button
            onClick={() => setViewMode('log')}
            className={`p-2 rounded-lg transition-colors ${viewMode === 'log' ? 'bg-primary text-zinc-900' : 'text-zinc-400 hover:text-white'}`}
          >
            <Plus size={20} />
          </button>
          <button
            onClick={() => setViewMode('calendar')}
            className={`p-2 rounded-lg transition-colors ${viewMode === 'calendar' ? 'bg-primary text-zinc-900' : 'text-zinc-400 hover:text-white'}`}
          >
      ...
      {viewMode === 'log' ? (
        ...
        >
          {/* Calendar Header */}
          <div className="flex justify-between items-center mb-6 bg-surface p-4 rounded-2xl border border-white/5">
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isSelected ? 'bg-primary text-zinc-900 font-bold' : 'hover:bg-white/5 text-white'}
                    ${isToday && !isSelected ? 'border border-primary text-primary' : ''}
                  `}
                >
                  <span>{format(date, 'd')}</span>
                  {hasWorkout && (
                    <div className={`w-1.5 h-1.5 rounded-full mt-1 ${isSelected ? 'bg-zinc-900' : 'bg-primary'}`} />
                  )}
                </button>
              );
            })}
          ...
          <div className="flex-1 bg-surface/50 rounded-3xl p-6 border border-white/5 overflow-y-auto">
            <h4 className="text-lg font-display font-bold text-white mb-4">
              {selectedDate ? format(selectedDate, 'MM月dd日 訓練紀錄', { locale: zhTW }) : '請選擇日期'}
            </h4>
            
            {!selectedDate ? (
              <p className="text-zinc-500 text-sm text-center py-8">點擊日曆上的日期查看訓練紀錄</p>
            ) : selectedWorkouts.length === 0 ? (
              <p className="text-zinc-500 text-sm text-center py-8">這天沒有訓練紀錄 😴</p>
            ) : (
              <div className="space-y-4">
                {selectedWorkouts.map(session => (
                  <div key={session.id} className="bg-black/20 p-4 rounded-2xl border border-white/5">
